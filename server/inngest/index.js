...
const autoCheckOut = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if(!attendance?.checkOut){
        ...
        await step.sleepUntil("Wait-for-the-1-hour", new Date(new Date().getTime
        + 1 * 60 * 60 *1000))
...
const leaveApplicationReminder = inngest.createFunction(
  ...
  async ({event, step})=>{
    ...
    //Wait 24 hours
    await step.sleepUntil("wait-for-the-24-hours",new Date(new Date().getTime
        + 24 * 60 * 60 *1000))
...
const attendanceReminderCron = inngest.createFunction(
  ...
  {
    cron: "0 0 6 * * *", //06:00 UTC = 11:30 AM SLST 
  },

  async ({event, step})=>{
    //Step 1: Get today's date range (SLST)
    const today = await step.run("get-today-date", ()=>{
      const startUTC = new Date(new Date().toLocaleDateString
      ("en-CA", {timeZone: "Asia/Colombo"}) + "T00:00:00 +05:30")
    ...
    const onLeaveIds = await step.run("get-on-leave-ids",
      async()=>{
    ...
    const checkedInIds = await step.run("get-checked-in-ids",
      async ()=>{
        const attendances = await Attendance.find({
          date: {$gte: new Date(today.startUTC), $lt:new
            Date(today.endUTC)
          }
        }).lean();
    ...
    const absentEmployees = activeEmployees.filter((emp)=>!onLeaveIds.
    includes(emp._id) && !checkedInIds.includes(emp._id))

    //Step 6: send remains email
    ...
    return {totalActive: activeEmployees.length, onLeave:
      onLeaveIds.length, checkedIn: checkedInIds.length,
